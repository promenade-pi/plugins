...
import type { SocialNetworkPayload } from './types';
 ...
 */
export function payloadOf(value: unknown): SocialNetworkPayload | null {
  let v: any = value;
  for (let depth = 0; depth < 4 && v && typeof v === 'object' && 'result' in v; depth++) v = v.result;
  if (v && typeof v === 'object' && !Array.isArray(v.resources) && v.value) v = v.value;
  return v && Array.isArray(v?.resources) ? (v as SocialNetworkPayload) : null;
}
 ...
 */
export function bootView(render: (payload: SocialNetworkPayload) => React.ReactElement) {
  ...
  try {
    ...
    root.render(
      payload
        ? render(payload)
        : <Notice title="No computed network" body={
            'This Social Network has no result in memory — a derived artifact is '
            + 'recomputed rather than stored, so a reloaded session has to run the '
            + 'miner again before this view can draw anything.'
          } />
    );
  } catch (err) {
    console.error('[social-network] view failed to start', err);
