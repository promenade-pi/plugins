...
export interface PromenadeApi {
  ...
  /**
   * Pushes parameter values back into the host's own param state — the same
   * schema-declared keys the host feeds in via the `params` event, just
   * flowing the other way. Used by a view that draws its own interaction
   * (a clicked matrix cell, a bar) to commit through the same persisted
   * params object a manifest-declared control would.
   */
