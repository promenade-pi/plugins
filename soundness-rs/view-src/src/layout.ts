...
export function buildElkGraph(net: AcceptingPetriNetPayload): { graph: ElkGraph; arcs: PetriArc[] } {
  ...
  for (const transition of [...(net.activities ?? [])].sort((a, b) => a - b)) {
    children.push({ id: `t${transition}`, width: labelOf(net, transition) == null ? SILENT_W : TRANS_W, height: labelOf(net, transition) == null ? SILENT_H : TRANS_H });
  }
  ...
  return {
    graph: {
      id: 'root', layoutOptions: ocpnLayoutOptions, children,
