...
import type { ResourceProfilesPayload } from './types';
 ...
 */
export function payloadOf(value: unknown): ResourceProfilesPayload | null {
  let v: any = value;
  for (let depth = 0; depth < 4 && v && typeof v === 'object' && 'result' in v; depth++) v = v.result;
  if (v && typeof v === 'object' && !Array.isArray(v.profiles) && v.value) v = v.value;
  return v && Array.isArray(v?.profiles) ? (v as ResourceProfilesPayload) : null;
}
 ...
 */
export function bootView(render: (payload: ResourceProfilesPayload) => React.ReactElement) {
  ...
  try {
    ...
    root.render(
      payload
        ? render(payload)
        : <Notice title="No computed profiles" body={
            'These Resource Profiles have no result in memory — a derived artifact '
            + 'is recomputed rather than stored, so a reloaded session has to run the '
            + 'analysis again before this view can draw anything.'
          } />
    );
  } catch (err) {
    console.error('[resource-behavior] view failed to start', err);
