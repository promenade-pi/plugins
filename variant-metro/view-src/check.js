...
// invariant. The entry is `metro-layout`'s own suite, so a change to the
// shared layout is caught here exactly as it is in the Metro Map plugin.
const esbuild = require('esbuild');
const { execFileSync } = require('child_process');
const os = require('os');
const path = require('path');

const out = path.join(os.tmpdir(), `variant-metro-check-${process.pid}.cjs`);
esbuild.buildSync({
