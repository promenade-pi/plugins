...
import { useComplexityFilter } from './complexity';
import { MetroLoader } from './MetroLoader';
import { defaultViewParams, formatDuration, type MetroMapPayload, type MmEdge, type MmNode, type ViewParams } from './types';
...
/** A view saved before 0.13.2 persisted a boolean `showNumbers`. */
...
function App({ map }: { map: MetroMapPayload }) {
  ...
  // The complexity slider position, or `null` = "use the computed seed"
  // (a deliberately sparse default). First user move pins it.
  const [complexity, setComplexity] = useState<number | null>(null);

  const basis = map.basis ?? 'petriNet';
  ...
  const filter = useComplexityFilter({
    basis, nodes: typeVisibleNodes, edges: typeVisibleEdges, byId, complexity,
  });
  ...
  const keptEdges = useMemo(
    () => [
      ...
      ...filter.extraEdges.filter((e) => keptIds.has(e.source) && keptIds.has(e.target)),
    ],
    [typeVisibleEdges, filter.keptEdgeIds, filter.extraEdges, keptIds],
  );
  ...
  // `routeAll` and `refineDrawing` all treat their input as read-only — the
  // logistics check asserts exactly that ("mutated ranked layout") — so there
  // is nothing for a per-stage copy to protect.
  ...
  const nodes: Node[] = useMemo(
    () => visibleNodes.map((n) => nodeFor(n, drawing.nodeX.get(n.id) ?? laneX(n.lane, pitch), y, theme, selected, highlightedTypes, params)),
    [visibleNodes, drawing, pitch, y, theme, selected, highlightedTypes, params],
  );

  const edges: Edge[] = useMemo(() => {
    const labelOf = (e: MmEdge): string | undefined => {
      if (params.edgeLabel === 'frequency') {
        return e.frequency != null ? e.frequency.toLocaleString('en') : undefined;
      }
      if (params.edgeLabel === 'performance') {
        return e.durationSecs != null ? formatDuration(e.durationSecs) : undefined;
      }
      return undefined;
    };
  ...
  }, [drawing, visibleEdgesAll, byId, highlightedTypes, params, theme]);
      ...
      >
        ...
        <Panel position="top-left">
          <Legend
            ...
            highlighted={highlightedTypes} onToggleHighlight={toggleHighlight} theme={theme} basis={basis}
          />
        </Panel>
        <Panel position="top-right">
          <ControlsPanel
            basis={basis} theme={theme} params={params}
            onParamsChange={(next) => setParams({ ...paramsStore.get(), ...next })}
            complexity={complexity ?? filter.seed} sliderMax={filter.sliderMax}
            onComplexityChange={setComplexity} coverage={filter.coverage}
          />
        </Panel>
      </ReactFlow>
    </div>
...
function nodeFor(
  ...
  if (n.kind === 'station') {
    ...
    const data: StationNodeData = {
      ...
      style: params.stationStyle, count: n.meta?.frequency, showNumber: params.edgeLabel === 'frequency',
    };
  ...
  const data: BoundaryNodeData = {
    ...
    count: n.meta?.frequency, showNumber: params.edgeLabel === 'frequency',
  };
  return {
...
function Legend({ map, hidden, setHidden, open, setOpen, highlighted, onToggleHighlight, theme, basis }: {
  ...
  theme: Record<string, string>; basis: 'petriNet' | 'directlyFollows';
}) {
    ...
    <div style={{ background: theme['bg-soft'], border: `1px solid ${theme.border}`, borderRadius: 8, padding: 10, minWidth: 182, fontSize: 11, color: theme.text, boxShadow: '0 2px 8px rgba(0,0,0,.08)' }}>
      ...
      {open && <div style={{ marginTop: 8, paddingTop: 6, borderTop: `1px solid ${theme.border}`, color: theme['text-dim'] }}>
        {basis === 'directlyFollows'
          ? <>Based on the <b>OC-DFG</b>.</>
          : <>Based on the <b>Petri net</b>. × marks a choice, + marks real parallelism. The slider reveals the process core first and optional activities last. A dashed segment is a line's very first or very last edge; a line running backward up the page is a genuine loop (rework), not a mining artefact.</>}
      </div>}
    </div>
  );
}
 ...
 *
 * The manifest marks this view `livePreview`, so the host also opens it for
 * a discovery run that has not finished yet — the panel appears immediately
 * instead of after a minute of nothing but a top-bar progress bar. In that
 * case `value` is the run's seed (the inputs), not an `OCMetroMap`, and
 * `runState` is 'running'. The host swaps the whole panel for the real
 * artifact's tab when the run lands, so this branch never has to become the
 * map itself; it only has to wait well.
 *
 * The payload check is not redundant with `runState`: a genuinely empty or
 * malformed artifact reaches the same standby screen rather than an
 * exception thrown out of the layout pass.
 */
function Root() {
  ...
  if (pending || !laidOut) {
    ...
    return (
      <MetroLoader
        theme={theme}
        title={pending ? 'Discovering the metro map…' : 'Drawing the metro map…'}
        message={pending
          ? (live.message || 'Mining the process model, then routing its lines.')
          : `Routing ${stations.toLocaleString('en')} stations and their connections.`}
...
// before anything renders — `App`'s first render is the multi-second layout
// pass this whole screen exists to cover, so it is far too late to do it
// from there.
promenade.ready();
